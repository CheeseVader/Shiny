...
function Get-ScopedDiffCheck([string]$Root){
  Push-Location $Root
  try{
    $raw=@(& cmd.exe /d /s /c 'git diff --check -- "backend/src/routes/auth.js" "backend/src/routes/admin.js" "frontend/src/pages/LoginPage.jsx" "frontend/src/pages/AdminPage.jsx" 2>&1')
    return @(
      $raw |
...
Write-Host " SHINY AUTH USUARIO + CONTRASENA R62E" -ForegroundColor Cyan
...
# R62D: captura errores preexistentes de git diff --check antes de tocar nada.
$baselineDiff=@(Get-ScopedDiffCheck $ProjectRoot)
if($baselineDiff.Count -gt 0){
  Write-Host ""
  Write-Host "Observaciones preexistentes antes de R62E:" -ForegroundColor Yellow
  $baselineDiff | ForEach-Object { Write-Host "  $_" -ForegroundColor DarkYellow }
}

$stamp=Get-Date -Format "yyyyMMdd-HHmmss"
$BackupRoot=Join-Path $ProjectRoot "_gmx_backups\AUTH-USERNAME-R62E-$stamp"
...
'@
  ...
  # R62E: remove trailing spaces/tabs introduced by removing the recovery link.
  # This is whitespace-only cleanup in LoginPage.jsx.
  $loginText=[regex]::Replace($loginText,'(?m)[ \t]+$','')
  $loginText=$loginText.TrimEnd("`r","`n")+"`r`n"

  Write-Utf8NoBom $Login $loginText
  Write-Host "[OK] Portal: Usuario + Contraseña; whitespace limpio." -ForegroundColor Green
  ...
  $adminPageText=[regex]::Replace($adminPageText,'(?m)[ \t]+$','')
  $adminPageText=$adminPageText.TrimEnd("`r","`n")+"`r`n"
  ...
  Write-Host ""
  Write-Host "=== GIT DIFF CHECK R62E ===" -ForegroundColor Cyan

  $afterDiff=@(Get-ScopedDiffCheck $ProjectRoot)
  $newDiffErrors=@(
    $afterDiff | Where-Object {
      $line=$_
      -not ($baselineDiff -contains $line)
    }
  )

  if($newDiffErrors.Count -gt 0){
    Write-Host "Errores NUEVOS introducidos por R62E:" -ForegroundColor Red
    $newDiffErrors | ForEach-Object { Write-Host "  $_" -ForegroundColor Red }
    throw "R62E introdujo errores nuevos de git diff --check."
  }

  if($afterDiff.Count -gt 0){
    Write-Host "[OK] R62E no agrego errores nuevos; las observaciones restantes ya existian." -ForegroundColor Yellow
  } else {
  ...
  Write-Host " R62E INSTALADO CORRECTAMENTE" -ForegroundColor Green
  ...
  Write-Host "Rollback R62E completado." -ForegroundColor Yellow
  throw
}
