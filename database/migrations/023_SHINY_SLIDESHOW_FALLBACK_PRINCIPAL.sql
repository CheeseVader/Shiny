...
WHERE b.row_id=c.row_id
  AND NOT EXISTS (
    SELECT 1 FROM shiny.cms_banners x
    WHERE x.zona=b.zona AND x.fallback_principal=true
  );

CREATE UNIQUE INDEX IF NOT EXISTS uq_cms_banner_fallback_por_zona
  ON shiny.cms_banners(zona)
  WHERE fallback_principal=true;

INSERT INTO shiny.schema_migrations(version,description)
SELECT '023','Shiny Fase Local 10.5.8 - slideshow fallback principal por zona'
WHERE NOT EXISTS(SELECT 1 FROM shiny.schema_migrations WHERE version='023');
