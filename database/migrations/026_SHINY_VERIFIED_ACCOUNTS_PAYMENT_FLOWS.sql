...
UPDATE shiny.cliente_cuentas SET email_verificado=true WHERE email_verificado=false;

CREATE TABLE IF NOT EXISTS shiny.cliente_email_tokens (
...
CREATE INDEX IF NOT EXISTS ix_cliente_email_tokens_account
  ON shiny.cliente_email_tokens(id_cuenta,expires_at);

CREATE TABLE IF NOT EXISTS shiny.payment_transactions (
  row_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS ix_payment_transactions_order
  ON shiny.payment_transactions(id_pedido,fecha_creacion DESC);
CREATE INDEX IF NOT EXISTS ix_payment_transactions_provider_session
  ON shiny.payment_transactions(provider_session_id)
...
SELECT '026','Shiny Fase Local 10.6.2 - verified accounts payment flows admin products'
WHERE NOT EXISTS(SELECT 1 FROM shiny.schema_migrations WHERE version='026');
