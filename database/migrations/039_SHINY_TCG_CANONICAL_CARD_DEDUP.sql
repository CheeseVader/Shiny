...
-- Build the same canonical identity Shiny will use during future syncs.
UPDATE shiny.tcg_master_cards
...
CREATE TEMP TABLE shiny_card_canonical_map ON COMMIT DROP AS
SELECT
  row_id,
  canonical_key,
...
CREATE INDEX ON shiny_card_canonical_map(row_id);
CREATE INDEX ON shiny_card_canonical_map(keeper_id);
...
WITH refs AS (
  ...
  FROM shiny_card_canonical_map m
  JOIN shiny.tcg_master_cards c ON c.row_id=m.row_id
  GROUP BY m.keeper_id
)
UPDATE shiny.tcg_master_cards k
...
INSERT INTO shiny.tcg_card_price_current(
...
FROM shiny.tcg_card_price_current p
JOIN shiny_card_canonical_map m ON m.row_id=p.master_card_id
WHERE m.row_id<>m.keeper_id
ON CONFLICT(master_card_id,price_provider,variant,currency) DO UPDATE SET
  low=CASE WHEN EXCLUDED.fetched_at>=shiny.tcg_card_price_current.fetched_at THEN EXCLUDED.low ELSE shiny.tcg_card_price_current.low END,
  mid=CASE WHEN EXCLUDED.fetched_at>=shiny.tcg_card_price_current.fetched_at THEN EXCLUDED.mid ELSE shiny.tcg_card_price_current.mid END,
  high=CASE WHEN EXCLUDED.fetched_at>=shiny.tcg_card_price_current.fetched_at THEN EXCLUDED.high ELSE shiny.tcg_card_price_current.high END,
  market=CASE WHEN EXCLUDED.fetched_at>=shiny.tcg_card_price_current.fetched_at THEN EXCLUDED.market ELSE shiny.tcg_card_price_current.market END,
  trend=CASE WHEN EXCLUDED.fetched_at>=shiny.tcg_card_price_current.fetched_at THEN EXCLUDED.trend ELSE shiny.tcg_card_price_current.trend END,
  source_url=CASE WHEN EXCLUDED.fetched_at>=shiny.tcg_card_price_current.fetched_at THEN EXCLUDED.source_url ELSE shiny.tcg_card_price_current.source_url END,
  provider_updated_at=GREATEST(shiny.tcg_card_price_current.provider_updated_at,EXCLUDED.provider_updated_at),
  fetched_at=GREATEST(shiny.tcg_card_price_current.fetched_at,EXCLUDED.fetched_at),
  source_hash=CASE WHEN EXCLUDED.fetched_at>=shiny.tcg_card_price_current.fetched_at THEN EXCLUDED.source_hash ELSE shiny.tcg_card_price_current.source_hash END;

-- History has no unique restriction, so it can simply be reassigned.
UPDATE shiny.tcg_card_price_history h
...
UPDATE shiny.tcg_cartas c
SET master_card_id=m.keeper_id
FROM shiny_card_canonical_map m
WHERE c.master_card_id=m.row_id
  AND m.row_id<>m.keeper_id;

UPDATE shiny.tcg_inventario i
SET master_card_id=m.keeper_id
FROM shiny_card_canonical_map m
...
DELETE FROM shiny.tcg_card_price_current p
USING shiny_card_canonical_map m
