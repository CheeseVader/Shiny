...
-- GMX Fase Local 10.5.1
-- CMS Visual + promociones transaccionales + fidelidad
BEGIN;

CREATE TABLE IF NOT EXISTS gmx.cms_banners (
...
CREATE INDEX IF NOT EXISTS idx_cms_banners_runtime
  ON gmx.cms_banners(zona,publicado,activo,fecha_inicio,fecha_fin,prioridad);

CREATE TABLE IF NOT EXISTS gmx.promociones_redenciones (
  row_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_promociones_redenciones_promo
  ON gmx.promociones_redenciones(id_promocion,estado,fecha);
CREATE INDEX IF NOT EXISTS idx_promociones_redenciones_pedido
  ON gmx.promociones_redenciones(id_pedido,estado);

CREATE TABLE IF NOT EXISTS gmx.fidelidad_cuentas (
  row_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS gmx.fidelidad_movimientos (
  row_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_fidelidad_mov_cliente
  ON gmx.fidelidad_movimientos(id_cliente,fecha DESC);
CREATE INDEX IF NOT EXISTS idx_fidelidad_mov_pedido
  ON gmx.fidelidad_movimientos(id_pedido,tipo);
...
SELECT '017','GMX Fase Local 10.5.1 - CMS banners promociones POS fidelidad'
WHERE NOT EXISTS(SELECT 1 FROM gmx.schema_migrations WHERE version='017');
