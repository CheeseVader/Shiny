...
CREATE TABLE IF NOT EXISTS shiny.tcg_sync_providers (
...
CREATE TABLE IF NOT EXISTS shiny.tcg_sync_game_config (
  row_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  game_code TEXT NOT NULL UNIQUE,
...
CREATE TABLE IF NOT EXISTS shiny.tcg_master_cards (
  row_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS ix_tcg_master_cards_game_set
  ON shiny.tcg_master_cards(game_code,set_code,name);

CREATE TABLE IF NOT EXISTS shiny.tcg_card_price_current (
  row_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  master_card_id BIGINT NOT NULL REFERENCES shiny.tcg_master_cards(row_id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS shiny.tcg_card_price_history (
  row_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  master_card_id BIGINT NOT NULL REFERENCES shiny.tcg_master_cards(row_id) ON DELETE CASCADE,
  price_provider TEXT NOT NULL,
  variant TEXT NOT NULL DEFAULT 'default',
...
CREATE INDEX IF NOT EXISTS ix_tcg_price_history_card_date
  ON shiny.tcg_card_price_history(master_card_id,fetched_at DESC);

ALTER TABLE shiny.tcg_cartas
...
INSERT INTO shiny.tcg_sync_providers(
...
INSERT INTO shiny.tcg_sync_game_config(game_code)
SELECT codigo FROM shiny.tcg_master_juegos
ON CONFLICT(game_code) DO NOTHING;

GRANT SELECT,INSERT,UPDATE,DELETE ON
  shiny.tcg_sync_providers,
  shiny.tcg_sync_game_config,
  shiny.tcg_master_cards,
  shiny.tcg_card_price_current,
  shiny.tcg_card_price_history
TO shiny_app;
GRANT USAGE,SELECT ON ALL SEQUENCES IN SCHEMA shiny TO shiny_app;

INSERT INTO shiny.schema_migrations(version,description)
SELECT '034','Shiny 10.6.2.3 - selective TCG auto sync, master cards, images and multi-source prices'
WHERE NOT EXISTS(SELECT 1 FROM shiny.schema_migrations WHERE version='034');
