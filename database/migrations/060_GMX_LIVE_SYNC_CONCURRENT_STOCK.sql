...
BEGIN
  IF NOT EXISTS(SELECT 1 FROM pg_constraint WHERE conname='ck_inventory_branch_stock_nonnegative') THEN
    ALTER TABLE gmx.inventario_sucursales
  ...
  IF NOT EXISTS(SELECT 1 FROM pg_constraint WHERE conname='ck_tcg_branch_stock_nonnegative') THEN
    ALTER TABLE gmx.tcg_inventario_sucursales
...
SELECT '060','GMX 10.6.2.4.1.12.2.0.3.6 - SSE live sync and concurrent stock protection'
WHERE NOT EXISTS(SELECT 1 FROM gmx.schema_migrations WHERE version='060');
