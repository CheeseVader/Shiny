...
INSERT INTO shiny.tcg_master_juegos(codigo,nombre,publisher,sitio_oficial,activo,orden)
VALUES
...
INSERT INTO shiny.tcg_master_sets(id_juego,codigo,nombre,fecha_lanzamiento,activo,fuente_oficial)
VALUES
...
INSERT INTO shiny.tcg_master_sets(id_juego,codigo,nombre,activo,fuente_oficial)
VALUES
...
INSERT INTO shiny.tcg_master_sets(id_juego,codigo,nombre,fecha_lanzamiento,total_cartas,activo,fuente_oficial)
VALUES
...
INSERT INTO shiny.tcg_master_rarezas(id_juego,codigo,nombre,activo,orden)
VALUES
...
CREATE OR REPLACE FUNCTION shiny.validate_tcg_card_scope()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
DROP TRIGGER IF EXISTS trg_tcg_inventory_scope_guard ON shiny.tcg_inventario;
CREATE TRIGGER trg_tcg_inventory_scope_guard
BEFORE INSERT OR UPDATE OF id_carta,rareza
ON shiny.tcg_inventario
FOR EACH ROW
EXECUTE FUNCTION shiny.sync_tcg_inventory_scope();

-- Verificación adicional en snapshots de adquisiciones.
CREATE OR REPLACE FUNCTION shiny.validate_tcg_acquisition_scope()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
DROP TRIGGER IF EXISTS trg_tcg_acquisition_scope_guard ON shiny.tcg_adquisiciones;
CREATE TRIGGER trg_tcg_acquisition_scope_guard
BEFORE INSERT OR UPDATE OF id_carta,id_juego,id_set,rareza
ON shiny.tcg_adquisiciones
FOR EACH ROW
EXECUTE FUNCTION shiny.validate_tcg_acquisition_scope();

INSERT INTO shiny.schema_migrations(version,description)
SELECT '031','Shiny 10.6.2.2 - Riftbound FFTCG Palworld and strict TCG scope integrity'
WHERE NOT EXISTS(SELECT 1 FROM shiny.schema_migrations WHERE version='031');
