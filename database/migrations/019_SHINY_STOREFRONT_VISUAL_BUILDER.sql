...
CREATE TABLE IF NOT EXISTS shiny.appearance_revisions (
...
GRANT SELECT,INSERT,UPDATE,DELETE ON TABLE shiny.appearance_revisions TO shiny_app;
GRANT USAGE,SELECT ON ALL SEQUENCES IN SCHEMA shiny TO shiny_app;

INSERT INTO shiny.schema_migrations(version,description)
SELECT '019','Shiny Fase Local 10.5.3 - storefront visual builder'
WHERE NOT EXISTS(SELECT 1 FROM shiny.schema_migrations WHERE version='019');
