...
-- Shiny 10.6.2.4.1.4.1
-- Fuente operativa única de stock = shiny.inventario_sucursales.
-- shiny.productos.stock queda como total agregado de compatibilidad/UI.

CREATE OR REPLACE FUNCTION shiny.recalculate_product_total_stock(p_product_id TEXT)
RETURNS VOID
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION shiny.sync_product_total_stock_from_inventory()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
  IF TG_OP='DELETE' THEN
    PERFORM shiny.recalculate_product_total_stock(OLD.id_producto);
    RETURN OLD;
  END IF;

  PERFORM shiny.recalculate_product_total_stock(NEW.id_producto);
...
ON shiny.inventario_sucursales(id_producto,id_sucursal,stock);

INSERT INTO shiny.schema_migrations(version,description)
SELECT '045','Shiny 10.6.2.4.1.4.1 - Unified multisucursal stock source and automatic aggregate'
WHERE NOT EXISTS(
  SELECT 1 FROM shiny.schema_migrations WHERE version='045'
);
