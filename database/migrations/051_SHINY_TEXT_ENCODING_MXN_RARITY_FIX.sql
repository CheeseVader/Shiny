...
SET valor =
  CASE
    WHEN shiny.configuracion.valor IS NULL
      OR shiny.configuracion.valor = ''
      OR shiny.configuracion.valor LIKE '%' || U&'n\00C3\00BAmero' || '%'
    THEN EXCLUDED.valor
    ELSE shiny.configuracion.valor
  END;

INSERT INTO shiny.schema_migrations(version,description)
SELECT
  '051',
  'Shiny 10.6.2.4.1.10.2.2 - text encoding, MXN labels and TCG rarity display'
WHERE NOT EXISTS(
  SELECT 1
