...
ALTER TABLE gmx.notificaciones_admin
...
UPDATE gmx.notificaciones_admin
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_notificaciones_open_key
  ON gmx.notificaciones_admin(clave)
  WHERE clave IS NOT NULL AND COALESCE(resuelta,false)=false;

CREATE INDEX IF NOT EXISTS ix_notificaciones_scope_open
  ON gmx.notificaciones_admin(id_sucursal,resuelta,prioridad,fecha DESC);

CREATE INDEX IF NOT EXISTS ix_notificaciones_type_open
  ON gmx.notificaciones_admin(tipo,resuelta,fecha DESC);

INSERT INTO gmx.configuracion(parametro,valor) VALUES
...
WHERE UPPER(COALESCE(p.modulo,'')) IN ('REPORTES','INVENTARIO','COMERCIAL','CAJA','TCG')
  AND NOT EXISTS(
    SELECT 1 FROM gmx.permisos_admin n
    WHERE LOWER(n.email)=LOWER(p.email) AND UPPER(n.modulo)='NOTIFICACIONES'
  );

GRANT SELECT,INSERT,UPDATE,DELETE ON TABLE gmx.notificaciones_admin TO gmx_app;

INSERT INTO gmx.schema_migrations(version,description)
SELECT '057','GMX 10.6.2.4.1.12.1.9 - complete notifications and alerts center'
WHERE NOT EXISTS(SELECT 1 FROM gmx.schema_migrations WHERE version='057');
