...
CREATE TABLE IF NOT EXISTS shiny.tcg_master_sets (
  row_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS shiny.tcg_master_rarezas (
  row_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  id_juego TEXT NOT NULL,
  codigo TEXT NOT NULL,
  nombre TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS shiny.importaciones (
  row_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
GRANT SELECT,INSERT,UPDATE,DELETE ON
  shiny.tcg_master_juegos,shiny.tcg_master_sets,shiny.tcg_master_rarezas,shiny.importaciones
TO shiny_app;
GRANT USAGE,SELECT ON ALL SEQUENCES IN SCHEMA shiny TO shiny_app;

INSERT INTO shiny.schema_migrations(version,description)
SELECT '030','Shiny 10.6.2.2 - TCG master catalog and Excel imports'
WHERE NOT EXISTS(SELECT 1 FROM shiny.schema_migrations WHERE version='030');
