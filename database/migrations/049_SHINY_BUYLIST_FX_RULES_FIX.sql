...
CREATE TABLE IF NOT EXISTS shiny.fx_rates (
...
CREATE INDEX IF NOT EXISTS ix_fx_rates_pair_date
  ON shiny.fx_rates(base_currency,quote_currency,rate_date DESC,fetched_at DESC);
...
GRANT SELECT,INSERT,UPDATE,DELETE ON shiny.fx_rates TO shiny_app;
GRANT USAGE,SELECT ON ALL SEQUENCES IN SCHEMA shiny TO shiny_app;

INSERT INTO shiny.schema_migrations(version,description)
SELECT '049','Shiny 10.6.2.4.1.10.1 - Buylist daily USD/MXN FX and catalog driven rules'
WHERE NOT EXISTS(SELECT 1 FROM shiny.schema_migrations WHERE version='049');
