...
try{
  ...
  const pool=new Pool({
    ...
    database:process.env.PGDATABASE||'shiny_db',user:process.env.PGUSER||'shiny_app',
    password:process.env.PGPASSWORD
  });
  const existing=await pool.query(`SELECT row_id,id_admin FROM shiny.administradores WHERE LOWER(email)=$1 ORDER BY row_id LIMIT 1`,[email]);
  if(existing.rowCount){
    await pool.query(`UPDATE shiny.administradores SET nombre=$2,password_hash=$3,rol='SUPERADMIN',activo=true,fecha_actualizacion=NOW()
      WHERE row_id=$1`,[existing.rows[0].row_id,name||'Administrador',hashPassword(password)]);
    console.log(`SUPERADMIN actualizado: ${email}`);
  }else{
    await pool.query(`INSERT INTO shiny.administradores(id_admin,nombre,email,password_hash,rol,activo,fecha_creacion,fecha_actualizacion,sucursales_permitidas)
