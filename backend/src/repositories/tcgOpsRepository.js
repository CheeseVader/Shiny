...
export async function quickScan(input, user) {
  ...
  try {
    ...
    const mid = await movement(client, { type, variant: v,
      ...
      globalBefore: gb, globalAfter: ga, reference: input.reference || '', reason: input.reason || 'Movimiento rÃ¡pido', user });
...
export async function createTcgSale(input, user) {
  ...
  try {
    ...
    let customer = { nombre: 'PÃºblico general', telefono: null, email: null };
    if (customerId) {
    ...
    for (const x of normalized) {
      ...
      [x.v.rareza, x.v.idioma, x.v.condicion, x.v.edicion].filter(Boolean).join(' Â· ')]
      );
  ...
  try {
    ...
    for (const d of details.rows) {
      ...
      await movement(client, { type: 'CANCELACION_VENTA_TCG_POS', variant: v, dest: b, qty,
        ...
        reason: input.reason || 'CancelaciÃ³n venta TCG', user });
    }
    if (String(sale.metodo_pago || '').toUpperCase() === 'EFECTIVO') {
      ...
      await client.query(`INSERT INTO gmx.caja_movimientos(
        ...
        'CancelaciÃ³n / reembolso TCG','TCG_POS',$7,$8,$9,false)`, [
    ...
    await reverseBenefitsTx(client, { order: sale, user, reason: input.reason || 'CancelaciÃ³n TCG' });
    ...
    await client.query(`UPDATE gmx.pedidos SET estado_pedido='CANCELADO',venta_confirmada=false,
      ...
      WHERE row_id=$1`, [sale.row_id, `CancelaciÃ³n TCG: ${input.reason || 'Sin motivo'}`]);
    await client.query('COMMIT');
    ...
    for (const d of details.rows) {
      ...
      if (input.applyAdjustments && diff !== 0) {
        ...
        await movement(client, { type: 'CONTEO_AJUSTE', variant: v,
          ...
          globalBefore: gb, globalAfter: ga, reference: id, reason: 'ConciliaciÃ³n conteo fÃ­sico TCG', user });
...
export async function applyCountAdjustment(id, user) {
  ...
  try {
    ...
    // Si el ajuste ya fue aplicado, devolver PASS l�gico sin modificar
    // inventario, movimientos ni auditor�a, aunque el conteo ya est� AJUSTADO.
