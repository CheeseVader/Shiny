...
import { createSale } from '../repositories/ordersRepository.js';
import { query, pool } from '../db.js';

const RUN_ID = Date.now();
const SALE_REQUEST_ID = `POS003-MP-R5-${RUN_ID}`;
const PROVIDER_PAYMENT_ID = `POS003-MP-R5-PAY-${RUN_ID}`;
...
async function cleanup() {
  section('POS-003-TEST1-R5 CLEANUP');
  ...
  try {
    ...
    if (selected?.type === 'TCG') {
      ...
      const tcgMov = await client.query(
      ...
      console.log(`CLEANUP=TCG_MOVIMIENTOS ${tcgMov.rowCount} ELIMINADOS`);
...
try {
  section(brandText("Shiny POS-003-TEST1-R5"));
  console.log('VENTA TARJETA / MERCADO PAGO');
  console.log('FLUJO REAL createSale()');
  console.log('PROVIDER SIMULADO');
  console.log('FIX: CONTRATO REAL CART ITEM inventoryId / productId');
    ...
    const tcg = await query(
      `SELECT
       ...
       WHERE COALESCE(s.stock,0)>=1
         AND COALESCE(g.stock,0)>=1
         AND COALESCE(
               NULLIF(g.precio_oferta,0),
               g.precio,
               0
             )>0
       ORDER BY
         COALESCE(NULLIF(g.precio_oferta,0),g.precio,0) DESC,
         s.stock DESC
       LIMIT 20`
    );
  ...
  /*
   * createSale() cart contract:
   * PRODUCT -> productId | itemId
   * TCG     -> inventoryId | itemId
   * quantity is mandatory.
   */
  const item = selected.type === 'PRODUCT' ?
  {
    itemType: 'PRODUCT',
    productId: selected.id_producto,
    quantity: 1
  } :
  {
    itemType: 'TCG',
    inventoryId: selected.id_inventario,
    quantity: 1
  };
  ...
  const sale = await createSale({
    ...
    notes: 'POS-003 TEST R5 CONTROLADO',
    user: {
      id_admin: 'LOCAL',
      nombre: 'POS-003 TEST',
      email: 'pos003-test@shiny.local'
  ...
  section('POS-003-TEST1-R5 RESULTADO');
  ...
  console.log('');
  console.log('POS-003-TEST1-R5=PASS');

} catch (e) {
  section('POS-003-TEST1-R5 FAIL');
  ...
  console.log('');
  console.log('POS-003-TEST1-R5 FINALIZADO');
}
