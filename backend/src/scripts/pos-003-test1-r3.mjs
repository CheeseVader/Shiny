...
const SALE_REQUEST_ID = `POS003-MP-R3-${RUN_ID}`;
const PROVIDER_PAYMENT_ID = `POS003-MP-R3-PAY-${RUN_ID}`;
...
async function cleanup() {
  section('POS-003-TEST1-R3 CLEANUP');
...
try {
  section(brandText("Shiny POS-003-TEST1-R3"));
  ...
  const sale = await createSale({
    ...
    notes: 'POS-003 TEST R3 CONTROLADO',
    user: {
      id_admin: 'LOCAL',
      nombre: 'POS-003 TEST',
      email: 'pos003-test@shiny.local'
  ...
  section('POS-003-TEST1-R3 RESULTADO');
  ...
  console.log('');
  console.log('POS-003-TEST1-R3=PASS');

} catch (e) {
  section('POS-003-TEST1-R3 FAIL');
  ...
  console.log('');
  console.log('POS-003-TEST1-R3 FINALIZADO');
}
