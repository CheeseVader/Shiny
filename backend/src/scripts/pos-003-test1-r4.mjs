...
const SALE_REQUEST_ID = `POS003-MP-R4-${RUN_ID}`;
const PROVIDER_PAYMENT_ID = `POS003-MP-R4-PAY-${RUN_ID}`;
...
async function cleanup() {
  section('POS-003-TEST1-R4 CLEANUP');
...
try {
  ...
  const sale = await createSale({
    ...
    notes: 'POS-003 TEST R4 CONTROLADO',
    user: {
      id_admin: 'LOCAL',
      nombre: 'POS-003 TEST',
      email: 'pos003-test@shiny.local'
  ...
  section('POS-003-TEST1-R4 RESULTADO');
  ...
  console.log('');
  console.log('POS-003-TEST1-R4=PASS');

} catch (e) {
  section('POS-003-TEST1-R4 FAIL');
  ...
  console.log('');
  console.log('POS-003-TEST1-R4 FINALIZADO');
}
