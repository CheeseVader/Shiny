-- Shiny PRODUCTOS AUDIT HOTFIX V1
...
BEGIN
  IF EXISTS (
    ...
    RAISE EXCEPTION 'SHINY_PRODUCT_SKU_DUPLICATES_EXIST: resuelve los SKU duplicados antes de crear el índice.';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    CREATE UNIQUE INDEX ux_productos_sku_normalizado
      ON shiny.productos (UPPER(TRIM(sku)))
      WHERE NULLIF(TRIM(COALESCE(sku,'')),'') IS NOT NULL;
  END IF;
END $$;
