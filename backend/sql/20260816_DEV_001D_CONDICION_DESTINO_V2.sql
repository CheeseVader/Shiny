...
UPDATE shiny.devoluciones_detalle AS dd
SET
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname='ck_devoluciones_detalle_condicion'
      AND conrelid='shiny.devoluciones_detalle'::regclass
  ) THEN
    ALTER TABLE shiny.devoluciones_detalle
...
ON shiny.devoluciones_detalle(condicion_articulo,destino_articulo);

GRANT SELECT,INSERT,UPDATE,DELETE
ON TABLE shiny.devoluciones_detalle
TO shiny_app;
...
FROM pg_constraint
WHERE conrelid='shiny.devoluciones_detalle'::regclass
  AND conname IN (
