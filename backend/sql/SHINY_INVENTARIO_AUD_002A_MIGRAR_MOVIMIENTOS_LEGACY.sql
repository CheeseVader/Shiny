-- Shiny INVENTARIO AUD-002A
-- Migración segura e idempotente del historial legacy.
-- NO elimina shiny.movimientos_inventario.
...
SELECT
  ...
  NULL,
  COALESCE(NULLIF(TRIM(m.usuario),''),NULLIF(TRIM(m.origen),''),'Shiny Legacy'),
  NULLIF(TRIM(m.usuario),''),
  COALESCE(NULLIF(TRIM(m.origen),''),'LEGACY_MOVIMIENTO')
FROM shiny.movimientos_inventario m
WHERE NULLIF(TRIM(COALESCE(m.id_movimiento,'')),'') IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM shiny.movimientos_inventario_sucursales s
    WHERE s.id_movimiento=m.id_movimiento
  );

COMMIT;

SELECT COUNT(*) AS legacy_total FROM shiny.movimientos_inventario;
