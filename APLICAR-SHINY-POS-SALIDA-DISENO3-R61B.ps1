...
Write-Host " SHINY POS - SALIDA PROTEGIDA DISENO 3 R61B" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan
...
$BackupRoot=Join-Path $ProjectRoot "_gmx_backups\POS-EXIT-DISENO3-R61B-$stamp"
...
'@
  ...
  if([regex]::IsMatch($cssText,$pattern)){
    $cssText=[regex]::Replace($cssText,$pattern,($block.Trim()+"`r`n"),1)
  } else {
    $cssText=$cssText.TrimEnd("`r","`n"," ","`t")+"`r`n`r`n"+$block.Trim()+"`r`n"
  }
  ...
  Write-Host ""
  Write-Host "=== GIT DIFF CHECK R61B ===" -ForegroundColor Cyan
  ...
  try{
    ...
    if($LASTEXITCODE -ne 0){
      throw "git diff --check detecto errores en R61B."
    }
  ...
  Write-Host " R61B INSTALADO CORRECTAMENTE" -ForegroundColor Green
  ...
  Write-Host "Rollback R61B completado." -ForegroundColor Yellow
  throw
}
