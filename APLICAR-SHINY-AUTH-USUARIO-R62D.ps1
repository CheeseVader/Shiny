...
Write-Host " SHINY AUTH USUARIO + CONTRASENA R62D" -ForegroundColor Cyan
...
if($baselineDiff.Count -gt 0){
  Write-Host ""
  Write-Host "Observaciones preexistentes antes de R62D:" -ForegroundColor Yellow
  $baselineDiff | ForEach-Object { Write-Host "  $_" -ForegroundColor DarkYellow }
}

$stamp=Get-Date -Format "yyyyMMdd-HHmmss"
$BackupRoot=Join-Path $ProjectRoot "_gmx_backups\AUTH-USERNAME-R62D-$stamp"
...
'@
  ...
  Write-Host ""
  Write-Host "=== GIT DIFF CHECK R62D ===" -ForegroundColor Cyan
  ...
  if($newDiffErrors.Count -gt 0){
    Write-Host "Errores NUEVOS introducidos por R62D:" -ForegroundColor Red
    $newDiffErrors | ForEach-Object { Write-Host "  $_" -ForegroundColor Red }
    throw "R62D introdujo errores nuevos de git diff --check."
  }

  if($afterDiff.Count -gt 0){
    Write-Host "[OK] R62D no agrego errores nuevos; las observaciones restantes ya existian." -ForegroundColor Yellow
  } else {
  ...
  Write-Host " R62D INSTALADO CORRECTAMENTE" -ForegroundColor Green
  ...
  Write-Host "Rollback R62D completado." -ForegroundColor Yellow
  throw
}
