...
if (src.includes('SHINY_POS_PAGO_MIXTO_001_V5')) {
  console.log('POS-PAGO-MIXTO-001 V5 ya estaba aplicado.');
  process.exit(0);
}

const stamp = new Date().toISOString().replace(/[-:TZ.]/g,'').slice(0,14);
const backup = `${file}.POS_PAGO_MIXTO_001_V5_${stamp}.bak`;
...
const failedCandidate = path.join(
  path.dirname(file),
  `OrdersPage.POS_PAGO_MIXTO_001_V5_CANDIDATE_${stamp}.jsx`
);
...
if (process.platform === 'win32') {
  // Ejecutar dentro de frontend evita problemas de --prefix y comillas en Windows.
  result = spawnSync(
...
console.log('POS-PAGO-MIXTO-001 V5 aplicado y compilado correctamente.');
