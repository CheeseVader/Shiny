...
if (src.includes('SHINY_POS_PAGO_MIXTO_001_V2')) {
  console.log('POS-PAGO-MIXTO-001 V2 ya estaba aplicado.');
  process.exit(0);
}

const stamp = new Date().toISOString().replace(/[-:TZ.]/g,'').slice(0,14);
const backup = `${file}.POS_PAGO_MIXTO_001_V2_${stamp}.bak`;
...
const result = spawnSync(npmCmd, ['run','build','--prefix',path.join(root,'frontend')], {
  cwd: root,
  encoding: 'utf8'
});
...
console.log('POS-PAGO-MIXTO-001 V2 aplicado y compilado correctamente.');
