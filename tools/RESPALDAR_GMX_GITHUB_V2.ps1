param(
  [string]$Root = "C:\Users\SrsGarciaEspinoza\Videos\GMX",
...
if (-not $reuse) {
  $dumpFile = Join-Path $dbDir ("gmx_db_" + $stamp + ".dump")
...
if (-not $staged) {
  ...
  if (-not $CommitMessage) {
    $CommitMessage = "backup: GMX completo + DB $stamp"
  }
...
Write-Host ""
Write-Host "RESPALDO GMX COMPLETADO." -ForegroundColor Green
