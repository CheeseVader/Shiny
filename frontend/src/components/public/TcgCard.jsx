...
export default function TcgCard({item,currency='MXN'}){
  ...
  return <article className="public-product-card gmx-product-card tcg-card">
    <Link to={`/tienda/tcg/item/${item.row_id}`} className="product-card-image">
      {item.imagen_principal?<img src={item.imagen_principal} alt={item.carta||''} loading="lazy"/>:<div className="public-image-placeholder gmx-image-fallback"><span>GMX</span><small>Imagen próximamente</small></div>}
    ...
    <div className="public-product-body">
      ...
      <div className="gmx-card-price-row"><div className="public-product-price">{money(item.precio,currency)}</div><span className={`gmx-stock-badge ${stock>0?'ok':'out'}`}>{stock>0?`${stock} disponible${stock===1?'':'s'}`:'Agotado'}</span></div>
