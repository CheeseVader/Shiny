...
Write-Host " SHINY AUTH USERNAME - DIAGNOSTICO R62" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan

$ProjectRoot=[System.IO.Path]::GetFullPath($ProjectRoot.TrimEnd('\'))
Assert-True (Test-Path -LiteralPath $ProjectRoot) "ProjectRoot no existe: $ProjectRoot"

$OutFile=Join-Path $ProjectRoot "DIAGNOSTICO-SHINY-AUTH-USERNAME-R62.txt"
...
$lines.Add("SHINY AUTH USERNAME - DIAGNOSTICO R62")
...
foreach($rel in $likely){
  ...
  $lines.Add(("{0,-8} {1}" -f ($(if(Test-Path -LiteralPath $p){"EXISTE"}else{"NO"}),$rel))
}
