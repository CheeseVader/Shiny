...
Write-Host " SHINY AUTH USUARIO + CONTRASENA R62B" -ForegroundColor Cyan
...
$BackupRoot=Join-Path $ProjectRoot "_gmx_backups\AUTH-USERNAME-R62B-$stamp"
...
'@
  ...
  Write-Host "[OK] Portal: Usuario + Contraseña." -ForegroundColor Green
  ...
  try{
    ...
    if($LASTEXITCODE -ne 0){ throw "git diff --check detecto errores R62B." }
  ...
  Write-Host " R62B INSTALADO CORRECTAMENTE" -ForegroundColor Green
  ...
  Write-Host "Rollback R62B completado." -ForegroundColor Yellow
  throw
}
