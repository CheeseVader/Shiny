...
function Get-DiffCheck([string]$Root){
  $args = 'diff --check -- "frontend/src/pages/OrdersPage.jsx" "frontend/src/pages/OrdersPagePOSRecommendedR78.css" 2>&1'
  Push-Location $Root
  try{
    $raw = @(& cmd.exe /d /s /c "git $args")
...
Write-Host " SHINY POS - FIX UTF8 DISENO 3 R61E" -ForegroundColor Cyan
Write-Host "============================================================" -ForegroundColor Cyan
...
$BackupRoot=Join-Path $ProjectRoot "_gmx_backups\POS-EXIT-R61E-$stamp"
...
# Baseline diff-check without PowerShell NativeCommandError on Git warnings.
$baselineResult = Get-DiffCheck $ProjectRoot
$baseline = @($baselineResult.Lines)

if($baseline.Count -gt 0){
  Write-Host ""
  Write-Host "Aviso: ya existen observaciones git diff --check ANTES de R61E:" -ForegroundColor Yellow
...
try{
  ...
  # ----------------------------------------------------------
  # 1) Fix only protected-exit modal content.
  # ----------------------------------------------------------
  ...
  # ----------------------------------------------------------
  # 2) Fix lock icon using CSS Unicode escape, no literal emoji.
  # ----------------------------------------------------------
  ...
  # Normalize EOF only, without rewriting style semantics.
  $cssText=$cssText.TrimEnd("`r","`n"," ","`t")+"`r`n"
  Write-Utf8NoBom $CssPath $cssText
  Write-Host "[OK] Icono candado corregido." -ForegroundColor Green

  # ----------------------------------------------------------
  # 3) Validate intended strings.
  # ----------------------------------------------------------
  ...
  # ----------------------------------------------------------
  # 4) Diff-check: reject only NEW real errors.
  # ----------------------------------------------------------
  Write-Host ""
  Write-Host "=== GIT DIFF CHECK R61E ===" -ForegroundColor Cyan

  $afterResult = Get-DiffCheck $ProjectRoot
  $after = @($afterResult.Lines)
  ...
  if($newErrors.Count -gt 0){
    Write-Host "Nuevos errores introducidos por R61E:" -ForegroundColor Red
    $newErrors | ForEach-Object { Write-Host "  $_" -ForegroundColor Red }
    throw "R61E introdujo errores nuevos de git diff --check."
  }

  if($after.Count -gt 0){
    Write-Host "[OK] R61E no agrego errores nuevos; las observaciones restantes ya existian." -ForegroundColor Yellow
  } else {
  ...
  Write-Host " R61E INSTALADO CORRECTAMENTE" -ForegroundColor Green
  ...
  Write-Host "Rollback R61E completado." -ForegroundColor Yellow
  throw
}
